...
Rokid::rokid_usb_thread(const std::atomic_bool& stop, std::atomic_bool& stopped)
{
	...
	_mutex.unlock();
	if (ok) {
		std::cout << std::format("Usb thread exiting normally");
	}
	else {
		std::cerr << std::format("Exiting on libusb error");
	}
	stopped = true;
}
...
WCHAR* getRokidDeviceString() {
	...
	std::cout << std::format("HidGuid = {{{:08x}-{:04x}-{:04x}-{:02x}{:02x}-{:02x}{:02x}{:02x}{:02x}{:02x}{:02x}}}\n",
	...
	std::cout << std::format("hDevInfoSet Handle: {}\n", hDevInfoSet);
	...
	if (hDevInfoSet != INVALID_HANDLE_VALUE) {
		...
		std::cerr << std::format("devInfo == INVALID_HANDLE_VALUE\n");
	}
...
Rokid::rokid_hmd_usb_init()
{
	...
	if (deviceString == NULL) {
		std::cerr << std::format("Failed to find Rokid Max");
		return false;
	}
	...
	if (INVALID_HANDLE_VALUE == _usb_dev)
	{
		...
		std::cerr << std::format("Failed to init HID: {}", getErrorCodeDescription(last_error));
		return false;
	}
...
bool Rokid::start() {
	std::cout << std::format("Starting Rokid driver instance");
	...
	std::cout << std::format("Started Rokid driver instance");
